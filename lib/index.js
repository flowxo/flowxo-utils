 ...
 * @param  {Object} [options] [optional nesting delimiter]
...
Utils.getFlattenedValue = function(data, path, options) {
  ...
  while(pieces.length && data != null) {
    let key = pieces.pop();
    let keyPieces = key.split(options.arrayDelimiter);
    if(keyPieces.length > 1 && options.arrayFormatter) {
      data = options.arrayFormatter(keyPieces.slice(1), data[keyPieces[0]]);
      continue;
    }
    data = data[key];
  }
